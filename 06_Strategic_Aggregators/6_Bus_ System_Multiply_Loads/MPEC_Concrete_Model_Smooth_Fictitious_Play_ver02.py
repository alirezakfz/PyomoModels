# -*- coding: utf-8 -*-
"""
Created on Mon Oct 11 15:03:54 2021
                                    ...
                                    sum(model.w_db_up[i,t]*F_d_b[i][t-16] for i in model.NCDA) +\
                                        sum(FMAX[i-1]*model.w_line_low[i,t] for i in model.LINES) +\
                                            sum(FMAX[i-1]*model.w_line_up[i,t] for i in model.LINES) -\
                                                sum(model.da_o_p[s,t]*supply_strategy[t-16][s-1]*10 for s in model.S) +\
                                                    sum(model.da_b_p[s,t]*demand_strategy[t-16][s-1] for s in model.S) for t in model.T )
                                                #                                                                         -\
