...
def load_data(file_index):
    df1 = pd.read_csv('prosumers_data/inflexible_profiles_scen_'+file_index+'.csv').round(5)*2 #*load_multiply #/1000
...
c_g[2]=[18 for x in range(0,horizon)]
c_g[3]=[20 for x in range(0,horizon)]
c_g[4]=[22 for x in range(0,horizon)]
...
for n in range(no_iteration+1):
    ...
    for j in range(1,ncda+2):
        ...
        charge_power = profiles['EV_Power']*load_multiply/2
        EV_soc_low   = profiles['EV_soc_low']*load_multiply/2
        EV_soc_up   = profiles['EV_soc_up']*load_multiply/2
        EV_soc_arrive = profiles['EV_soc_arr']*load_multiply/2
        EV_demand = profiles['EV_demand']*load_multiply/2
        
                
        # Shiftable loads
        SL_loads=[]
        SL_loads.append(profiles['SL_loads1']*load_multiply/2)
        SL_loads.append(profiles['SL_loads2']*load_multiply/2)
