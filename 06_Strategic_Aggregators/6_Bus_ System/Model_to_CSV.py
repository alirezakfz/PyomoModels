...
def model_to_csv_iteration(model, IN_loads, iteration, da_index, file_time_str, EVs_list):
    timestr = time.strftime("%Y%m%d-%H%M%S")
    ...
    if iteration ==0:
        row=['Iteration','DAs_load\nE_DAs_L', 'DAs_generation\nE_DAs_G', 'DAs_demand_bid\nb_t','DAs_supply_offer\no_t',\
             'EVs_Charge', 'EVs_Discharge', 'TCL_Power', 'SL_Power', 'Inflexible_loads','Solar_Power']
        write_mode='w'
    ...
    for t in model.T:
        ...
        row.append(sum(value(model.solar_power[i,t]) for i in model.N)/1000)
