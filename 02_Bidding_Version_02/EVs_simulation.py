...
def electric_vehicles(number_of_EVs=10, 
    ...
    x_arrival=get_truncated_normal(u_arrival, std_arrival, low=16, upp=25)
    arrival_time=[]
    
    u_depart    = 31#18
    std_depart  = 2#4.2, 1.73  
    x_depart=get_truncated_normal(u_depart, std_depart, low=29, upp=36)
    depart_time=[]
    ...
    for ev in EV_samples:
        ...
        while check:
            ...
            if  (arrive + dem_time < depart) and depart <= (time_slot+16)*slot:
                check=False
