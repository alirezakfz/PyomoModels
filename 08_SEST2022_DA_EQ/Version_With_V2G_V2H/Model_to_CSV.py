...
def model_to_csv(model, IN_loads):
    ...
    for t in model.T:
        ...
        row.append(sum(value(model.E_EV_CH[i,t]) for i in model.N if i in EVs_list)/1000)
        row.append(sum(value(model.E_EV_DIS[i,t]) for i in model.N if i in EVs_list)/1000)
...
def model_to_csv_iteration(model, IN_loads, iteration, da_index, file_time_str, EVs_list, load_multiply):
    ...
    if iteration ==0:
        row=['Iteration','DAs_load', 'DAs_generation', 'DAs_demand_bid','DAs_supply_offer',\
             'EVs_Charge', 'EVs_Discharge','EVs_2H','EVs_2G', 'TCL_Power', 'SL_Power', 'Inflexible_loads','Solar_Power','PV_2H','PV_2G']
        write_mode='w'
    ...
    for t in model.T:
        ...
        row.append(sum(value(model.E_EV_DIS2H[i,t]) for i in model.N if i in EVs_list)*load_multiply )
        row.append(sum(value(model.E_EV_DIS2G[i,t]) for i in model.N if i in EVs_list)*load_multiply )
        ...
        row.append(sum(value(model.E_PV_2H[i,t]) for i in model.N)*load_multiply )
        row.append(sum(value(model.E_PV_2G[i,t]) for i in model.N)*load_multiply )
    ...
    model_file='Model_CSV/Model_data_DA_'+da_index+'_'+timestr+'.csv'
    with open(model_file, write_mode, newline='') as file:
        csv_writer = writer(file)
        for rw in list_row:
                csv_writer.writerow(rw)
    ...
    dual_file='Model_CSV/Model_dual_variables_DA_'+da_index+'_'+timestr+'.csv'
    with open(dual_file, write_mode, newline='') as file:
        csv_writer = writer(file)
        for rw in list_row:
                csv_writer.writerow(rw)
