...
price_d_o=dict()
price_d_o['DAS']= random_price(horizon,10,15)

for i in range(1,ncda+1):
    price_d_o[i] = random_price(horizon,10,15)
...
def random_irrediance_solar_power(irrediance, in_loads, j, solar_list):
    ...
    for da in solar_list[j]:
        for i in range(horizon):
            ...
            solar_power[da-1,i] = 0.000157 * area * irrediance[i] * (1 - 0.0001*random.randint(4, 7)* (outside_temp[i]-25))*load_multiply
