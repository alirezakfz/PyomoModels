# -*- coding: utf-8 -*-
"""
Created on Fri Jul 16 16:18:57 2021
...
def mpec_model(ng, nb, nl, ncda, IN_loads, gen_capacity,
    ...
    def DA_demand_rule(model,t):
        # return sum(model.da_b_p[s,t] * demand_strategy[t-16][s-1] for s in model.S) <= bigF * model.u_DA[t] 
    ...
    def DA_supply_rule(model,t):
        # return sum(model.da_o_p[s,t] * supply_strategy[t-16][s-1] for s in model.S) <= bigF * (1-model.u_DA[t])
   ...
   # Constraint (C.1) generators dual price
    ...
    def KKT_stKrategic_DA_bid_up_rule (model,t):
        # return sum(model.da_o_p[s,t] * supply_strategy[t-16][s-1] for s in model.S) - model.E_DA_G[t] <= model.u_DAs_o_up[t] * bigM
    ...
    def KKT_stKrategic_DA_bid_up_3_rule (model,t):
        # return sum(model.da_o_p[s,t] * supply_strategy[t-16][s-1] for s in model.S) - model.E_DA_G[t] >= 0
    ...
    def KKT_strategic_demand_up_rule (model,t):
        # return sum(model.da_b_p[s,t] * demand_strategy[t-16][s-1] for s in model.S) - model.E_DA_L[t] <= model.u_DAs_b_up[t] * bigM
    ...
    def KKT_strategic_demand_up_3_rule (model,t):
        # return sum(model.da_b_p[s,t] * demand_strategy[t-16][s-1] for s in model.S) - model.E_DA_L[t] >= 0
    ...
    #                                     sum(FMAX[i-1]*model.w_line_low[i,t] for i in model.LINES) +\
    #                                         sum(FMAX[i-1]*model.w_line_up[i,t] for i in model.LINES)  for t in model.T ) -\
    #                                             sum(model.da_o_p[s,t]*supply_strategy[t-16][s-1]*10 for s in model.S for t in model.T) +\
    #                                                 sum(model.da_b_p[s,t]*demand_strategy[t-16][s-1] for s in model.S for t in model.T) # pyo.log10((model.da_p[s,t]))
    # model.obj = Objective(rule=social_welfare_optimization_rule, sense=minimize)
