...
def mpec_model(ng, nb, nl, ncda, IN_loads, gen_capacity,
    ...
    """
    Demand and Supply for each prosumers
    """
    model.pro_demand = Var(model.N, model.T, within=NonNegativeReals, initialize=0)
    model.pro_supply = Var(model.N, model.T, within=NonNegativeReals, initialize=0)
    model.u_pro = Var(model.N, model.T, within=NonNegativeReals, initialize=0)
    ...
    #********************************************************
    #             Prosumers Demand and Supply Constraints
    #********************************************************
    def prosumers_demand_load_rule(model, i, t):
        return model.pro_demand[i,t] - model.pro_supply[i,t]  == model.POWER_SL[i,t] + IN_loads.loc[i-1,str(t)] + model.POWER_TCL[i,t]*load_multiply + model.E_EV_CH[i,t] - model.E_EV_DIS[i,t] - model.solar_power[i,t]
    model.prosumers_demand_load_con = Constraint(model.N, model.T, rule=prosumers_demand_load_rule)
    
    # def prosumers_supply_load_rule(model, i, t):
    #     return model.pro_supply[i,t] == model.E_EV_DIS[i,t] + model.solar_power[i,t]
    # model.prosumers_supply_load_con = Constraint(model.N, model.T, rule=prosumers_supply_load_rule)
    
    def prosumers_power_balance_rule(model, i, t):
        return model.pro_demand[i,t] <= bigF * model.u_pro[i,t]
    model.prosumers_power_balance_con = Constraint(model.N, model.T, rule=prosumers_power_balance_rule)
    
    def prosumers_power_balance_rule2(model, i, t):
        return model.pro_supply[i,t] <= bigF * (1-model.u_pro[i,t])
    model.prosumers_power_balance_con2 = Constraint(model.N, model.T, rule=prosumers_power_balance_rule2)
    
    
    
    def total_demands_rule(model, t):
        return model.E_DA_L[t] - model.E_DA_G[t] == sum(model.pro_demand[i,t] for i in model.N)*PU_DA - sum(model.pro_supply[i,t] for i in model.N)*PU_DA
    model.total_demands_con = Constraint(model.T, rule=total_demands_rule)
    ...
    #     sum_total = sum( model.POWER_SL[i,t] + IN_loads.loc[i-1,str(t)] + model.POWER_TCL[i,t]*load_multiply + model.E_EV_CH[i,t]- model.E_EV_DIS[i,t]  for i in model.N)
