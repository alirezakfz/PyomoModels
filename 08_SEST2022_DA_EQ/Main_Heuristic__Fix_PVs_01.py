# -*- coding: utf-8 -*-
"""
Created on Thu Oct  7 17:05:23 2021
...
def check_bids(old, new, epsilon):
    ...
    for key in old.keys():
        ...
        if np.sum(compare) == len(old[key]):
            print("DA ", key," converged to epsilon difference")
            check=True
        else:
            return False
    return check
...
def select_best_action(j, objective_function, ncda, n_iteration ):
    ...
    for i in range(ncda+1):
        if counter != j:
            indexes = np.where(np.array(objective_function[counter][1:n_iteration])==min(objective_function[counter][1:n_iteration]))[0]
            
            # min_obj_index = np.argmin(objective_function[counter][1:n_iteration])+1
            min_obj_index = indexes[-1]+1 # Add one to bypass the first iteration ,
...
def random_irrediance_solar_power(irrediance, in_loads, j, solar_list):
    ...
    for da in solar_list[j]:
        for i in range(horizon):
            ...
            solar_power[da-1,i] = 0.000157 * area * irrediance[i] * (1 - 0.001*random.randint(4,8)* (outside_temp[i]-25))*load_multiply
    return solar_power
...
for n in range(no_iteration+1):
    ...
    for j in DAs_list:
        ...
        if (results.solver.status == SolverStatus.ok) and (results.solver.termination_condition == TerminationCondition.optimal):
            ...
            model_to_csv_iteration(model, IN_loads.sum(0), n, str(j), timestr, EVs_list[j],PU_DA*load_multiply)
