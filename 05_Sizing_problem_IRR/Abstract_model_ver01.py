...
def soc_increment_rule(model,i,t):
    if t >= model.arrival[i] and t < model.depart[i]:
        return model.SOC[i,t+1] ==  model.E_EV[i,t]*charge_rate #sum(model.u[i,j,t] for j in model.M)*model.E_EV[i,t]*charge_rate
    else:
        return Constraint.Skip
...
def one_job_rule(model,i):
    sumj=[]
    for j in model.M:
        time=range(model.arrival[i], model.depart[i] )
