...
def dataFile(number_of_EVs, 
    ...
    """
    Finding the priority for charging EVs as first come first serve
    sort based on their demand/chargerate and demand/duratiuon
    """
    
    priority=[]
    for i in range(ev_no):
        temp=demand[i]/EV_types[EV_samples[i]]["charge_rate"]
        temp=temp/(depart_time[i]-arrival_time[i])
        temp=round(temp,3)
        priority.append(temp)
        
    precedence=pd.DataFrame()
    precedence['Arrival']=arrival_time
    precedence['Priority']=priority
    
    precedence=precedence.sort_values(['Arrival','Priority'],ascending = (True, True))
    
    index=list(precedence.index.values)
    
    
    # priority=np.zeros((ev_no,ev_no))    
    
    # for ii in range(ev_no-1):
    #     i=index[ii]
    #     for jj in range(ii+1,ev_no):
    #         j=index[jj]
    #         priority[i,j]=1

    
    
    ##Reorder the parameters after sorting 
    arrival_time=[arrival_time[key] for key in index]
    depart_time=[depart_time[key] for key in index]
    distance=[distance[key] for key in index]
    demand=[demand[key] for key in index]
    EV_samples=[EV_samples[key] for key in index]
    ...
    return arrival_time, depart_time, distance, demand, charge_power,installed_chargers,\
             installed_cost,TFC, EV_samples,priority
