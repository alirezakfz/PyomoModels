...
def electric_vehicles(number_of_EVs=10, 
    ...
    u_arrival   = 19#7
    std_arrival = 2# 4.2,1.73
    x_arrival=get_truncated_normal(u_arrival, std_arrival, low=16, upp=24)
    arrival_time=[]
    
    u_depart    = 7#18
    std_depart  = 2#4.2, 1.73  
    x_depart=get_truncated_normal(u_depart, std_depart, low=5, upp=12)
    depart_time=[]
    ...
    for ev in EV_samples:
        ...
        while check:
            ...
            if depart> 1 and (dem_time<time_slot-arrive+depart) and arrive <=time_slot*slot:
                check=False
    ...
    return arrival_time, depart_time, distance , charge_power, EV_samples
