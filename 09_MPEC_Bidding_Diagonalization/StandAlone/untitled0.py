# -*- coding: utf-8 -*-
"""
Created on Fri Sep  9 17:17:56 2022
...
def mpec_model(ng, nb, nl, ncda, IN_loads, gen_capacity,
    ...
    model.NO_DAS = RangeSet(NO_DAs)
    
    # Energy bid as load from the grid
    model.E_DA_L = Var(model.NO_DAS, model.T, within=NonNegativeReals, initialize=0)
    
    # Energy bid as inject into grid
    model.E_DA_G = Var(model.NO_DAS, model.T, within=NonNegativeReals, initialize=0)
    
    # Energy bid demand by DA
    model.DA_demand = Var(model.NO_DAS, model.T,within=NonNegativeReals, initialize=0)
    
    # Energy supply by DA
    model.DA_supply = Var(model.NO_DAS, model.T,within=NonNegativeReals, initialize=0)
    
    # Binary control variable for demand and supply for DA
    model.u_DA = Var(model.NO_DAS, model.T,within=Binary, initialize=0)
    ...
    model.E_EV_CH = Var(model.NO_DAS, model.N, model.T, within=NonNegativeReals, initialize=0)
    
    # Energy from EV Discharge
    model.E_EV_DIS = Var(model.NO_DAS, model.N, model.T, within=NonNegativeReals, initialize=0)
    
    # EVs battery State of 
    model.SOC   = Var(model.NO_DAS, model.N, model.T, within=NonNegativeReals, initialize=0)
    
    # Binary control variable
    model.u_EV   = Var(model.NO_DAS, model.N, model.T, within=Binary, initialize=0)
    
    
    
    # ***********************************
    # Thermostatically appliances
    model.POWER_TCL = Var(model.NO_DAS, model.N, model.T, within=NonNegativeReals, initialize=0)
    
    # Room temprature control variable
    model.TCL_TEMP = Var(model.NO_DAS, model.N, model.T, within=NonNegativeReals, initialize=0)
    
    
    # ****************************************
    #  Shiftable loads
    model.POWER_SL = Var(model.NO_DAS, model.N, model.T, within=NonNegativeReals, initialize=0)
    
    # Binary control varible
    model.u_SL     = Var(model.NO_DAS, model.N, model.T, within=Binary, initialize=0)
    ...
    def solar_power_bounds(model,n, t):
        return (0, sum(solar_power[n,i-1,t-16] for i in model.N if i in Solar_list[n-1])) # *load_multiply
    model.solar_power=Var(model.NO_DAs, model.T,  within=NonNegativeReals,  bounds=solar_power_bounds, initialize=0)
    ...
    def ev_charging_rule(model,n, i,t):
        if t >= arrival[n-1][ i-1] and t < depart[n-1][i-1] :
            return model.E_EV_CH[n,i,t] <= model.u_EV[n,i,t] * charge_power[n][i-1]*delta_t
        else:
            return model.E_EV_CH[n,i,t]==0
        return model.E_EV_CH[n,i,t]==0 #Constraint.Skip
    model.ev_charging_con=Constraint(model.NO_DAS, model.N, model.T, rule=ev_charging_rule)
    
    # Constraint (a.3): Ensure that charging of EV don't exceed maximum value of EV_Power
    def ev_discharging_rule(model,n,i,t):
        if t >= arrival[n-1][i-1] and t < depart[n-1][i-1] :
            return model.E_EV_DIS[i,t] <= (1-model.u_EV[i,t]) * charge_power[i-1]*delta_t
        else:
            return model.E_EV_DIS[n,i,t]==0
        return model.E_EV_DIS[n,i,t]==0#Constraint.Skip
    model.ev_discharging_con=Constraint(model.NO_DAS, model.N, model.T, rule=ev_discharging_rule)
    
    # Constraint (4.3.1) EVS_list not participate in the market
    def ev_discharging_list_rule(model,n, i, t):
        if i not in EVs_list[n-1]:
            return model.E_EV_DIS[n,i,t]==0
        return Constraint.Skip
    model.ev_discharging_list_con=Constraint(model.NO_DAS, model.N, model.T, rule=ev_discharging_list_rule)
            
    
    # Constraint (a.4): set the SOC 
    def ev_soc_rule(model,n, i, t):
        if t >= arrival[n-1][i-1] and t < depart[n-1][i-1] : 
            return model.SOC[n,i,t+1] == model.SOC[n,i,t] + ch_rate*model.E_EV_CH[n,i,t] - model.E_EV_DIS[n,i,t]/ch_rate 
        else:
            return Constraint.Skip
    model.ev_soc_con = Constraint(model.NO_DAS, model.N, model.T, rule=ev_soc_rule)
    
    # Constraint (a.4_1): Set the start soc to arrival soc
    def EV_arrival_soc_rule(model, n, i):
        return model.SOC[n, i, arrival[n-1][i-1]]== EV_soc_arrive[n-1][i-1]
    model.EV_arrival_soc_con = Constraint(model.NO_DAS, model.N, rule= EV_arrival_soc_rule)
    
    # Constraint (a.5_1): Limit the SOC, Lower Bound
    def ev_soc_low_rule(model,n, i, t):
        if t >= arrival[n-1][i-1] and t < depart[n-1][i-1]: 
            return model.SOC[n,i,t] >=  EV_soc_low[n-1][i-1]
        else:
            return Constraint.Skip
    model.ev_soc_low_con=Constraint(model.NO_DAS, model.N, model.T, rule=ev_soc_low_rule)
    
    
    # Constraint (a.5_2): Limit the SOC, Upper Bound
    def ev_soc_low2_rule(model,n, i, t):
        if t >= arrival[n-1][i-1] and t < depart[n-1][i-1]: 
            return model.SOC[n,i,t] <=  EV_soc_up[n-1][i-1]
        else:
            return Constraint.Skip
    model.ev_soc_low2_con=Constraint(model.NO_DAS,model.N, model.T, rule=ev_soc_low2_rule)
    
    # Constraint (a.6): Set the target SOC to be as desired(full charge) at departure time
    def ev_traget_rule(model,n,i):
        return model.SOC[n,i,depart[n-1][i-1]] == EV_soc_up[n-1][i-1]   
    model.ev_target_con = Constraint(model.NO_DAS, model.N, rule=ev_traget_rule)
    
    # Constraint (Custom_1): Set the binary variable to zero outside the [arrival, depart] boundary
    def ev_binary_zero_rule(model, n ,i,t):
        if t < arrival[n-1][i-1] or t >= depart[n-1][i-1]:
             return model.u_EV[n,i,t]==0
        else:
             return Constraint.Skip
    model.ev_binary_zero_con = Constraint(model.NO_DAS, model.N, model.T, rule=ev_binary_zero_rule)
    ...
    def TCL_power_limit_rule(model,n, i,t):
        return model.POWER_TCL[n,i,t] <= TCL_Max[n-1][i-1] 
    model.TCL_power_limit_con= Constraint(model.NO_DAS, model.N, model.T, rule=TCL_power_limit_rule)
    
    # Constraint (a.8): Set inside temprature for residence
    def TCL_room_temp_rule(model,n,i,t):
        ...
        if t >= arrival[n-1][i-1] and t < depart[n-1][i-1] :                                                                 # model.TCL_occ[i,t]
            return model.TCL_TEMP[n,i,t+1]== TCL_Beta[n-1][i-1] * model.TCL_TEMP[n,i,t] + (1-TCL_Beta[n-1][i-1])*(outside_temp[n-1][t-16]+ TCL_R[n-1][i-1]*model.POWER_TCL[n,i,t])
        else:
            return model.POWER_TCL[n,i,t] == 0
            # return Constraint.Skip
    model.TCL_room_temp_con= Constraint(model.NO_DAS, model.N, model.T, rule=TCL_room_temp_rule)
    
    # Constraint (a.9):
    def TCL_low_preference_rule(model,n,i,t):
        ...
        # #     return Constraint.Skip
        if t >= arrival[n-1][i-1] and t < depart[n-1][i-1] :
            return model.TCL_TEMP[n,i,t] >= TCL_temp_low[n-1][i-1]
        else:
            return Constraint.Skip
    model.TCL_low_preference_con = Constraint(model.NO_DAS, model.N, model.T, rule=TCL_low_preference_rule)
    
    # Constraint (a.9_1): Set the temperature of the room to the outside temp
    def TCL_set_start_temp_rule(model,n, i):
            return model.TCL_TEMP[n, i,arrival[n-1][i-1]]== TCL_temp_low[n-1][i-1]# model.out_temp[model.arrival[i]]
    model.TCL_set_start_temp_con= Constraint(model.NO_DAS, model.N, rule=TCL_set_start_temp_rule)
    ...
    def SL_power_load_rule(model,n, i, t):
        if t >= (SL_low[n-1][i-1]) and t < SL_up[n-1][i-1] :
            time = range(0,SL_cycle) # Cycle duration required to finish the assigned jop for SL 
            # time = range(16,SL_cycle + 16)
            return model.POWER_SL[n,i,t] ==  sum(SL_loads[n-1][w][i-1]* model.u_SL[n,i,t-w] for w in time if w+16<=t )
        else:
            return model.POWER_SL[n,i,t] == 0
            # return Constraint.Skip
    model.SL_power_load_con = Constraint(model.NO_DAS, model.N, model.T, rule=SL_power_load_rule)
    
    # Constraint (a.11): The begining time for cycle
    def SL_binary_rule(model, n, i):
        time = range(SL_low[n-1][i-1], SL_up[n-1][i-1])
        return sum(model.u_SL[n,i,t] for t in time) ==1
    model.SL_binary_con = Constraint(model.NO_DAS, model.N, rule=SL_binary_rule)
    
    # Constraint (19_1): Zeros less than lower time to statrt
    def SL_binary_zero_rule(model,n, i ,t):
        if t < SL_low[n-1][i-1]:
            return model.u_SL[n,i,t]==0
        elif t >= SL_up[n-1][i-1]:
            return model.u_SL[n,i,t]==0
        else:
            return Constraint.Skip
    model.SL_binary_zero_con =Constraint(model.NO_DAS, model.N, model.T, rule=SL_binary_zero_rule)
