...
if RandomOrExcel=='E'
    ...
    EV_SOC_max = (xlsread([currentFolder,'\',DatasetList{DatasetSelection},'\EVs.xlsx'],'max_soc',xlRC2A1(2,2,Large_Random_Number+1,2+nsda-1))).*dem_multiplier;
    ...
    EV_SOC_min = (xlsread([currentFolder,'\',DatasetList{DatasetSelection},'\EVs.xlsx'],'min_soc',xlRC2A1(2,2,max(nev)+1,2+nsda-1))).*dem_multiplier;
    ...
    EV_Initial_SOC = (xlsread([currentFolder,'\',DatasetList{DatasetSelection},'\EVs.xlsx'],'Arrival SOC',xlRC2A1(2,2,max(nev)+1,2+nsda-1))).*dem_multiplier;%/MVA;
    ...
    SL_profile = xlsread([currentFolder,'\',DatasetList{DatasetSelection},'\SL.xlsx'],'SL Consumption',xlRC2A1(2,2,max(nsl)+2,T*nsda+1)).*dem_multiplier;;
...
RES = (xlsread([currentFolder,'\',DatasetList{DatasetSelection},'\Renewable Production.xlsx'],'Sheet1',strcat('B2:Y',LastRow))).*dem_multiplier;%./MVA);%.*dem_multiplier;