...
% 'Ali Data':      1
% 'Konster Data':  2
DatasetList = {'Ali Data','Konster Data'};
DatasetSelection = 1;
...
for ii=1:max([nev,ntcl,nsl])*nda
    ...
    if ii<=max(ntcl)*nda
        if TCL_start_vec(ii)>0
            a8(ii,TCL_start_vec(ii)+1:TCL_end_vec(ii)) = tcl_temp(ii,TCL_start_vec(ii)+1:TCL_end_vec(ii)) ==  TCL_beta_vec(ii)*tcl_temp(ii,TCL_start_vec(ii):TCL_end_vec(ii)-1)+(1-TCL_beta_vec(ii))*(outside_temp(TCL_start_vec(ii):TCL_end_vec(ii)-1)+TCL_COP_vec(ii)*TCL_R_vec(ii)*(p_tcl(ii,TCL_start_vec(ii):TCL_end_vec(ii)-1)));%MVA*1000*
