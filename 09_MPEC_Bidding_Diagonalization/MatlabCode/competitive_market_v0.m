        ...
        if SL_start_vec(ii)>0
            ...
            for tt=1:T
                if tt>=SL_start_vec(ii)&&tt<=SL_end_vec(ii)
                    if tt-SL_cycle_vec<0 % <= or <
                        a10(ii,tt) = p_sl(ii,tt) == w(ii,1:tt)*SL_profile_vec(ii,1:tt)';
                    else
                        a10(ii,tt) = p_sl(ii,tt) == w(ii,tt-[0:SL_cycle_vec-1])*SL_profile_vec(ii,1:SL_cycle_vec)';
                    end
                else
            ...
            for tt=1:T
                if tt>=SL_start_vec(ii)&&tt<=SL_end_vec(ii)
                    if tt-SL_cycle_vec<0 % <= or <
                        ...
                        lin_a10(ii,tt) = p_sl(ii,tt) == lin_w(ii,tt-[0:SL_cycle_vec-1])*SL_profile_vec(ii,1:SL_cycle_vec)';
                    end
                else
